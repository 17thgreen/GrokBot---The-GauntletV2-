"""CB-VEL Coinbase 1m velocity incremental maps vs MKT-KALSHI-15M-MID.

Frozen no-fit map for DRAFT-FEAT-20260913-005 composed with
DRAFT-ABST-20260913-005. Speak only on T-14m headlines under Policy B
(bar_end < decision_time). No CF. No Map 2. No Φ(z). No Poly last.
No Policy A. No annex. No trading.
"""
...
W_SCORED = 0.0015
LAMBDA_SCORED = 0.15
LAMBDA_ROBUST = (0.10, 0.15, 0.20)
W_ROBUST = (0.0010, 0.0015, 0.0025)
...
def last_completed_start_policy_b(dec_ts: int) -> int:
    ...
    Bar start unix seconds for the last completed CB-001 1m candle.
    """
...
def gate_decision(
    ...
    *,
    ...
    """DRAFT-ABST-20260913-005 eligibility. Policy B only. No annex."""
...
class CBVELRow:
    ...
    prior_ok: bool
    not_locked: bool
    speak: bool  # True iff ALLOW_SPEAK (feature applies velocity clip)
...
def apply_map(
    rows: Sequence[CBVELRow],
    *,
    lam: float = LAMBDA_SCORED,
    w: float = W_SCORED,
    v_override: Optional[Sequence[Optional[float]]] = None,
) -> list[float]:
    """CB-VEL frozen map. ABSTAIN / missing → p=m; else clipped velocity pull.

    v_override used for placebos only.
    """
    out: list[float] = []
    for i, r in enumerate(rows):
        ...
        ramp = _clip(float(v) / w, -1.0, 1.0)
        out.append(_clip(r.m_t + lam * ramp, EPS, 1.0 - EPS))
    return out
...
def score_forecast(
    ...
    *,
    ...
    if speak_idx:
        ...
        v_speak = [rows[i].v for i in speak_idx if rows[i].v is not None]
        mean_v_speak: Any = (
            float(sum(v_speak) / len(v_speak)) if v_speak else "UNTESTED"
        )
    else:
        mean_m_speak = "UNTESTED"
        mean_p_speak = "UNTESTED"
        mean_v_speak = "UNTESTED"
...
def placebo_lambda_zero(rows: Sequence[CBVELRow]) -> list[float]:
    """λ=0 diagnostic — must give Δ=0."""
    return apply_map(rows, lam=0.0)


def placebo_flip_sign_v(rows: Sequence[CBVELRow], cell_id: str) -> list[float]:
    """Flip sign of v; should not beat the true sign."""
    ordered = sorted(
    ...
    for r in ordered:
        if r.v is None:
            ...
            overrides.append(-float(r.v))
    return apply_map(
        ordered, lam=LAMBDA_SCORED, w=W_SCORED, v_override=overrides
    )


def placebo_shuffle_v(rows: Sequence[CBVELRow], cell_id: str) -> list[float]:
    """Shuffle v within cell leaving m fixed."""
    ordered = sorted(
        rows, key=lambda r: (r.contract_id, r.decision_time, r.time_remaining_sec)
    )
    v_vals = [r.v for r in ordered]
    rng = random.Random(PLACEBO_SEED + 100 + CELL_SEED_OFFSET.get(cell_id, 0))
    shuffled = v_vals[:]
    rng.shuffle(shuffled)
    return apply_map(
        ordered, lam=LAMBDA_SCORED, w=W_SCORED, v_override=shuffled
    )
